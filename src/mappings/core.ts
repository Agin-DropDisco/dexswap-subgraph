...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = DexSwap.load(getFactoryAddress())
...
export function handleSync(event: Sync): void {
  ...
  let dexswapp = DexSwap.load(getFactoryAddress())

  // reset factory liquidity by subtracting onluy tarcked liquidity
  dexswapp.totalLiquidityNativeCurrency = dexswapp.totalLiquidityNativeCurrency.minus(
  ...
  dexswapp.totalLiquidityNativeCurrency = dexswapp.totalLiquidityNativeCurrency.plus(trackedLiquidityNativeCurrency)
  dexswapp.totalLiquidityUSD = dexswapp.totalLiquidityNativeCurrency.times(bundle.nativeCurrencyPrice)
...
export function handleMint(event: Mint): void {
  ...
  dexswapp.txCount = dexswapp.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let dexswapp = DexSwap.load(getFactoryAddress())
  dexswapp.totalVolumeUSD = dexswapp.totalVolumeUSD.plus(trackedAmountUSD)
  dexswapp.totalVolumeNativeCurrency = dexswapp.totalVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
  dexswapp.untrackedVolumeUSD = dexswapp.untrackedVolumeUSD.plus(derivedAmountUSD)
  dexswapp.txCount = dexswapp.txCount.plus(ONE_BI)
  ...
  let dexswappDayData = updateDexSwapDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  dexswappDayData.dailyVolumeUSD = dexswappDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  dexswappDayData.dailyVolumeNativeCurrency = dexswappDayData.dailyVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
  dexswappDayData.dailyVolumeUntracked = dexswappDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  dexswappDayData.save()
